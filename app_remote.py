...
def upload_content():
    ...
    if api_key:
        try:
            ...
            ai_text_for_result = content[:2000] if content else ''
            if not ai_text_for_result and saved_files:
                for sf in saved_files:
                    fpath = os.path.join(UPLOAD_FOLDER, sf)
                    txt = extract_text_from_file(fpath)
                    if txt:
...
@app.route('/api/admin/upload/<int:upload_id>', methods=['GET'])
@admin_required
def admin_get_upload(upload_id):
    db = get_db(); u = db.execute('SELECT * FROM uploads WHERE id=?', (upload_id,)).fetchone()
    if not u: db.close(); return jsonify({'error': '内容不存在'}), 404
    d = dict(u)
    try: d['ai_result'] = json.loads(d['ai_result']) if d['ai_result'] else {}
    except: d['ai_result'] = {}
    db.close(); return jsonify(d)

@app.route('/api/admin/upload/edit-ai', methods=['POST'])
@admin_required
def admin_edit_upload_ai():
    data = get_json_body(); uid = to_int(data.get('upload_id'))
    if not uid: return jsonify({'error': '缺少upload_id'}), 400
    db = get_db(); u = db.execute('SELECT ai_result FROM uploads WHERE id=?', (uid,)).fetchone()
    if not u: db.close(); return jsonify({'error': '内容不存在'}), 404
    try: old = json.loads(u['ai_result']) if u['ai_result'] else {}
    except: old = {}
    old.update({'title': data.get('ai_title',''), 'disease': data.get('ai_disease',''), 'category': data.get('ai_category',''), 'preview': data.get('ai_preview',''), 'full_content': data.get('ai_full_content',''), 'types': data.get('ai_types',[])})
    db.execute('UPDATE uploads SET ai_result=? WHERE id=?', (json.dumps(old, ensure_ascii=False), uid))
    db.commit(); db.close()
    return jsonify({'message': 'AI内容已保存', 'ai_result': old})

@app.route('/api/admin/upload/check-duplicates', methods=['POST'])
@admin_required
def admin_check_duplicates():
    data = get_json_body(); uid = to_int(data.get('upload_id'))
    db = get_db(); u = db.execute('SELECT ai_result FROM uploads WHERE id=?', (uid,)).fetchone()
    if not u: db.close(); return jsonify({'error': '内容不存在'}), 404
    try: ai = json.loads(u['ai_result']) if u['ai_result'] else {}
    except: ai = {}
    title = (ai.get('title') or '').strip(); disease = (ai.get('disease') or '').strip()
    dup = []
    if title:
        for r in db.execute('SELECT id,title FROM treatments WHERE title=? AND status="published"', (title,)).fetchall():
            dup.append({'type': 'treatment', 'id': r['id'], 'title': r['title'], 'preview': '完整内容', 'match': '标题完全相同'})
        for r in db.execute('SELECT id,title,preview FROM treatments WHERE title LIKE ? AND status="published" LIMIT 5', ('%'+title[:6]+'%',)).fetchall():
            if r['title'] != title:
                dup.append({'type': 'treatment', 'id': r['id'], 'title': r['title'], 'preview': r['preview'] or '', 'match': '标题相似'})
    if disease:
        for r in db.execute('SELECT id,name FROM diseases WHERE name LIKE ?', ('%'+disease[:4]+'%',)).fetchall():
            tcnt = db.execute('SELECT COUNT(*) as cnt FROM treatments WHERE disease_id=?', (r['id'],)).fetchone()['cnt']
            dup.append({'type': 'disease', 'id': r['id'], 'title': r['name'], 'preview': '已有'+str(tcnt)+'个疗程', 'match': '疾病名称相关'})
    db.close(); return jsonify({'duplicates': dup, 'count': len(dup)})

@app.route('/api/admin/upload/import', methods=['POST'])
@admin_required
def admin_import_upload():
    data = get_json_body(); uid = to_int(data.get('upload_id'))
    rid = request.current_user['user_id']
    db = get_db(); u = db.execute('SELECT * FROM uploads WHERE id=?', (uid,)).fetchone()
    if not u: db.close(); return jsonify({'error': '内容不存在'}), 404
    if u['status'] == 'imported': db.close(); return jsonify({'error': '已入库'}), 400
    try: ai = json.loads(u['ai_result']) if u['ai_result'] else {}
    except: ai = {}

    # Filter out skipped titles if provided by admin
    skip_titles = data.get('skip_titles', '')
    if skip_titles:
        skip_list = [s.strip() for s in skip_titles.split(',') if s.strip()]
        items = ai.get('treatments', [ai] if 'title' in ai else [])
        cleaned = [t for t in items if (t.get('title') or '').strip() not in skip_list]
        if not cleaned:
            db.close(); return jsonify({'error': chr(20840)+chr(37096)+chr(20869)+chr(23481)+chr(24050)+chr(34987)+chr(36339)+chr(36807)+chr(65292)+chr(26080)+chr(21487)+chr(20837)+chr(24211)+chr(30340)+chr(25968)+chr(25454)}), 400
        if 'treatments' in ai:
            ai['treatments'] = cleaned
        elif 'title' in ai:
            ai = cleaned[0] if cleaned else ai
    ...
    from ai_processor import process_and_insert
    try:
        results = process_and_insert(get_db(), ai, u['user_id'] or rid)
    except Exception as e: return jsonify({'error': '入库失败: ' + str(e)}), 500
    db.execute('UPDATE uploads SET status="imported", reviewer_id=? WHERE id=?', (rid, uid))
    if u['user_id']:
        db.execute('UPDATE users SET points=points+100 WHERE id=?', (u['user_id'],))
        user = db.execute('SELECT points FROM users WHERE id=?', (u['user_id'],)).fetchone()
        if user:
            db.execute('INSERT INTO points_log (user_id, amount, balance_after, source_type, source_id, note) VALUES (?,100,?,?,?,?)',
                       (u['user_id'], user['points'], 'upload_import', uid, '入库奖励'))
    db.commit(); db.close()
    msg = '入库成功！'
    if len(results) == 1:
        msg += '创建了1个疗程方案'
    else:
        msg += '创建了'+str(len(results))+' 个疗程方案'
    rnames = []
    for r in results:
        rnames.append(r.get('title') or r.get('id',''))
    if rnames:
        msg += '：'+', '.join(rnames[:5])
    if len(rnames) > 5:
        msg += '等'
